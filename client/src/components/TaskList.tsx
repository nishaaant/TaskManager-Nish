...
const TaskList: React.FC = () => {
  ...
  return (
    <div className="p-6 bg-gray-100 min-h-screen">
      ...
      {showAddForm && <TaskForm onTaskAdded={() => { fetchAllTasks(); setShowAddForm(false); }} />}
            ...
            <tbody>
              {tasks.map((task) => (
                <tr key={task._id} className="text-center hover:bg-gray-50">
                  ...
                  <td className="border p-2">{((new Date(task.endTime).getTime() - new Date(task.startTime).getTime()) / (1000 * 60 * 60)).toFixed(2)}</td>
